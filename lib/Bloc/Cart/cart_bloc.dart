...
class CartBloc extends Bloc<CartEvent, CartState> {
  ...
  Future<void> _onResetQuantity(
      OnResetQuantityEvent evet, Emitter<CartState> emit) async {
    emit(state.copyWith(quantity: 1));
  }

  Future<void> _onAddProductToCart(
      OnAddProductToCartEvent event, Emitter<CartState> emit) async {
    final verify = product
        .where((pro) => pro.uidProduct.contains(event.productCart.uidProduct));
    ...
    if (verify.isEmpty) {
      ...
      emit(state.copyWith(
          products: product, quantityCart: product.length, total: total));
    }
  }
  ...
  Future<void> _onDecreaseQuantityProductToCart(
    ...
    emit(state.copyWith(
        products: product, quantityCart: product.length, total: total.abs()));
  }

  Future<void> _onClearCart(
      OnClearCartEvent event, Emitter<CartState> emit) async {
    emit(state.copyWith(
        amount: 0, quantity: 0, products: [], quantityCart: 0, total: 0.0));
  }
}
